...
async function handlePhotoButtonClick(event) {
    ...
    try {
        ...
        if (totalPhotos > 0) {
            openPhotoGalleryModal(groups, getPhotoModalTitle(groups, totalPhotos));
...
function openPhotoFullscreenGallery(links, startIndex) {
    const gallery = links.map(link => ({
        src: link.dataset.photoDownloadUrl || link.dataset.photoFull || link.href,
        ...
        custom: {
            ...
            fallbackUrl: link.dataset.photoFull || link.dataset.photoPreview || link.href,
            previewUrl: link.dataset.photoPreview || ''
        }
    }));
    ...
    window.Fancybox.show(gallery, {
        ...
        on: {
            ready: fancybox => {
                ...
                fancybox.container.addEventListener('error', event => {
                    const image = event.target && event.target.tagName === 'IMG'
                        ? event.target
                        : null;
                    const slide = fancybox.getSlide();
                    if (!image || !slide || !slide.custom || slide.custom.fallbackApplied) {
                        return;
                    }

                    const fallbackUrl = [
                        slide.custom.fallbackUrl,
                        slide.custom.previewUrl
                    ].find(url => (
                        url &&
                    ...
                    slide.custom.fallbackApplied = true;
                    slide.src = fallbackUrl;
...
function openPhotoGalleryModal(groups, title) {
    closePhotoModal();
...
function renderPhotoLink(photo, title) {
    const href = photo.downloadUrl || photo.fullUrl || photo.previewUrl;
